...
#    if data_back[len(data_back)-1]==np.nan:
#        data_back[len(data_back)-1]=0
    #Imputing zeros by nearest neighbours
#    data_f=dataset.replace(to_replace=0,method='ffill')
#    data_b=dataset.replace(to_replace=0,method='bfill')
#    dataset=(data_f.values + data_b.values) / 2
#    print(data_forward, data_back)

    new_data = (data_forward.values + data_back.values) / 2
    #Taking previous and next week/month/year
    #x = dataset.isnull()
#    dataset = dataset[0]
#    div_factor = 3
...
#    xyz_data=xyz.xyz_class(sku_price,total_price)
    ...
    #ABC based on volume
#    data_abc=pd.DataFrame(columns=['sku','abcofdata'])
#    abc_alter=abc.abc_class(datasets)

#    trained_outputs = []
    forecast_results = []
    num = 0
    output_all=pd.DataFrame(columns=['sku','model','forecast1','forecast2','forecast3','forecast4','forecast5','rmse','mape'])
...
#            output['Variability_Segment']=abc_alter[incr]
#        else:
#            output['Variability_Segment']=abc_data[incr]
#        output['Velocity_segment']=xyz_data[incr]
#        output['spn']=spn[incr]
...
#        if data_cluster[sku]==0:
#            value=npm.forecast_for_NPI(0,sku_data,plant_cluster,datasets)
#
#        print(xxxxxxxx)
...
#        print("Dimension: ", data.shape)
        ...
        for key in rmse_ML:
            model_forecast=model_predict(key, best_order,data, forecast_period)
            model_forecast = [0 if i < 0 else int(i) for i in model_forecast]
            rmse_val=np.mean(rmse_ML[key])
            mape_val=np.mean(calculate_validation_mape(expected, predictions[key]))
            output_all=output_all.append({'sku':sku,'model':key,'forecast1':model_forecast[0],'forecast2':model_forecast[1],'forecast3':model_forecast[2],'forecast4':model_forecast[3],'forecast5':model_forecast[4],'rmse':rmse_val,'mape':mape_val},ignore_index=True)
...
#            validation_ts[model] = predictions[model]
        ...
        output_all=output_all.append({'sku':sku,'model':'ensemble','forecast1':Ve[0],'forecast2':Ve[1],'forecast3':Ve[2],'forecast4':Ve[3],'forecast5':Ve[4],'rmse':rmse_val,'mape':mape_val},ignore_index=True)
#        error_en=calculate_rmse('Ensemble', expected, Ve)
...
#        if error_ml==0:
#            print("Error ML is zero ---> forecast ML")
#            forecast=ft.model_predict(best_models[0], best_order,data, forecast_period)
...
#            output['validation'] = ut.assign_dates(predictions[best_models[0]], 'validation', dataset.tail(5))
#            validation_facc = ft.calculate_validation_facc(expected, predictions[best_models[0]])
...
#            ft.plot_all_forecasts(dataset, sku_data, forecast, forecast, forecast, forecast, sku)
#            continue
#
#        if error_ts==0:
#            print("Error TS is zero ---> forecast TS")
#            forecast=ft.model_predict(best_models[1], best_order,data, forecast_period)
#            output['forecast_values'] = ut.assign_dates(forecast, 'forecast', dataset.tail(1))
#            output['facc'], output['mape'], output['bias'] = ft.calculate_forecast_accuracy(raw_data.tail(1).sales, forecast[0])
#            output['validation'] = ut.assign_dates(predictions[best_models[1]], 'validation', dataset.tail(5))
#            validation_facc = ft.calculate_validation_facc(expected, predictions[best_models[1]])
#            output['validation_facc'] = ut.assign_dates(validation_facc, 'val_facc', dataset.tail(5))
#
#            forecast_results = ft.output_forecast(sku, dataset, sku_data, output, forecast_results)
#            ft.plot_all_forecasts(dataset, sku_data, forecast, forecast, forecast, forecast, sku)
...
#if both errors are same the MA(3)
#        if error_ml==error_ts:
#            print("Errors Same ---> MA(3)")
#            forecast = mu.moving_average(sku_data[0],forecast_period,3)
#            output['forecast_values'] = forecast
#            output['facc'] = ft.calculate_forecast_accuracy(raw_data.iloc[-1], forecast[0])
#
#            forecast_results = ft.output_forecast(sku, datasets[sku].T, sku_data, output, forecast_results)
#            ft.plot_all_forecasts(dataset, sku_data, forecast, forecast, forecast, sku)
#            continue

#Weight calculation directly from error
#        weight_ts=ut.calculate_weight(error_ts,error_ml)
#        weight_ml=ut.calculate_weight(error_ml,error_ts)
        ...
        models_out=models_out.append({'sku':sku,'model_ts':best_models[1],'model_ml':best_models[0]},ignore_index=True)
...
#
    ...
    return forecast_results,output_all,models_out
...
#    plt.savefig(r'sku_' + str(sku) + '.png')
    plt.show()
...
def output_forecast(sku, dataset, sku_data, output, forecast_results):
    ...
    with open(r'Forecasting_data.json', 'w', encoding = 'utf-8') as f:
...
#%%
forecast_results, output_all,models_out = training(datasets, forecast_period)
output_all.to_csv(r'validationforecast.csv')

#%%
#output = output_dataframe(forecast_results, "FUSO_2000_Dec.csv")
